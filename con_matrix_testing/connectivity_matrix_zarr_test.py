...
#load dataset
data_xarray = xr.open_zarr('Trajectory_swain_sample.zarr')
...
sf = "../../GBR_reef_names/swain_sample/GBR_names_swain_sample.shp"
...
for p in range(0,len(polys)): # loop over polys
    ...
    for pp in range(0,len(startpoints)): # finding which particles start in that polygon
        ...
        i = i+1
    start[polys['LOC_NAME_S'][p]] = particle_list #store list in dictionary # SUSPICIOUS
#print(start)
#print(endpoints)
...
for p in range(0,len(polys)): # loop through polys
    ...
    for pp in range(0,len(polys)): #loop over all of the polys again
        for particle in particle_list: #for the particles which started in our "first" polygon
            if (polys['geometry'][pp].contains(endpoints['geometry'][particle])): # which ones finish in our "second" poly # SUSPICIOUS
...
plt.matshow(con_matrix, fignum=f.number)
plt.xticks(range(con_matrix.select_dtypes(['number']).shape[1]), con_matrix.select_dtypes(['number']).columns, fontsize=14, rotation=90)
plt.yticks(range(con_matrix.select_dtypes(['number']).shape[1]), con_matrix.select_dtypes(['number']).columns, fontsize=14)
cb = plt.colorbar()
